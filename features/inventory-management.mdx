...
|----------|-------------|----------|-------------------|
| `general` | Basic library items | Calculators, USB drives | No |
| `premium` | High-value items | Tablets, cameras | No |
| `computo` | Computing equipment | Laptops, monitors | No |
| `libro` | Books and reading material | Textbooks, novels | Yes (fee per day) |

<Info>
  **Access Control**: Premium users can access all categories. Cliente users cannot request `premium` category items.
</Info>
